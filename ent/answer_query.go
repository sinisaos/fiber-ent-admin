...
func (aq *AnswerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Answer, error) {
	...
	if aq.withQuestion != nil || aq.withAuthor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withAuthor; query != nil {
		if err := aq.loadAuthor(ctx, query, nodes, nil,
			func(n *Answer, e *User) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AnswerQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Answer)
	for i := range nodes {
